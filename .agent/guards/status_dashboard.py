...
读取 Agent OS 各数据源，生成结构化 Markdown 仪表盘。
可被工作流直接调用，也可命令行独立运行。
...
class StatusDashboard:
    ...
    def _header(self) -> str:
        return "# 📊 Antigravity Agent OS — System Dashboard\n"
    ...
    def _task_progress(self) -> str:
        ...
        # 解析各状态任务
        done_tasks = re.findall(r"\[✅ DONE\]\s+(T-\d+)", ctx_content)
        pending_tasks = re.findall(r"\[⏳ PENDING\]\s+(T-\d+)", ctx_content)
        progress_tasks = re.findall(r"\[🔄 IN_PROGRESS\]\s+(T-\d+)", ctx_content)
        blocked_tasks = re.findall(r"\[🚫 BLOCKED\]\s+(T-\d+)", ctx_content)
        failed_tasks = re.findall(r"\[❌ FAILED\]\s+(T-\d+)", ctx_content)
    ...
    def _evolution_stats(self) -> str:
        ...
        # 反思次数
        rl_content = self._read_file(self.evolution_dir / "reflection_log.md")
        reflection_count = len(re.findall(r"## \d{4}-\d{2}-\d{2} Session:", rl_content))
    ...
    def _get_last_reflection_date(self, content: str) -> str:
        """获取最后一次反思日期。"""
        dates = re.findall(r"## (\d{4}-\d{2}-\d{2}) Session:", content)

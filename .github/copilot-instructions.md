# Antigravity Agent OS — Copilot 全局配置

> 本文件为 GitHub Copilot 提供项目级指令，确保 Copilot 理解 Antigravity Agent OS 的工作方式。
...
### 1.1 项目级配置检测
当工作目录下存在 `.agents/` 目录时：
1. **读取记忆**: 立即读取 `.agents/memory/active_context.md`
...
| "确认" / "Go" / "OK" (用户版PRD后) | 技术评审 | 👨‍💻 技术总监评估可行性 |
| "确认" / "Go" / "OK" (研发版PRD后) | `feature-flow` | 🚀 启动递归执行流水线 |
